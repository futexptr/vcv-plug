...
void Client::updateDevices() {
	...
	for (auto& el : messageHandler.deviceList.Devices) {
		...
		if (el.DeviceMessages.size() > 0) {
			...
			for (auto& deviceMessage : el.DeviceMessages) {
				tempDevice.commandTypes.push_back(deviceMessage.CmdType);
				if (!deviceMessage.CmdType.compare(testSensor)) {
					for (auto& elCmdAttrs : deviceMessage.DeviceCmdAttributes) {
						tempDevice.sensorTypes.push_back(elCmdAttrs.SensorType);
					}
				}
			}
		}
